...
\begin{itemize}
    ...
    \item \textbf{Analyse graphique complète:} Les propriétés structurelles ont été analysées incluant la distribution des degrés (loi de puissance, $\alpha \approx 3.0$), l'assortativité thème-lien, la détection de communautés (modularité Louvain: 0.6476) et les mesures de centralité (Report-4).
    \item \textbf{Expériences de base GCN:} Les expériences initiales de prédiction de liens utilisant les Réseaux Convolutifs sur Graphes~\cite{kipf2017semi} ont atteint un AUC de test de 0.8653 avec arrêt précoce, démontrant que l'apprentissage topologique seul fournit un point de départ solide.
