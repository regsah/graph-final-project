...
Key findings include the surprising dominance of uncorrected Node2Vec, the
continued failure of naive uncorrected embedding concatenation, the competitive
performance of exact rank fusion between GCN and BGE-M3, and the strong
performance of purely structural GCN embeddings learned without any text features.
...
7 & GCN + BGE-M3 (rank fusion) & Fusion & Exact Reciprocal Rank Fusion of GCN and BGE-M3 rankings over the full shared candidate set \\
...
\midrule
BGE-M3 Only & 2.14\% & 10.20\% & 24.37\% & 33.08\% & 0.0506 & 0.1004 & 0.3308 & 0.0492 \\
Node2Vec (corrected) & 1.57\% & 7.95\% & 11.85\% & 12.98\% & 0.0351 & 0.0539 & 0.1298 & 0.0342 \\
BGE-M3 + Node2Vec (corrected) & 1.81\% & 9.82\% & 24.65\% & 33.83\% & 0.0471 & 0.0987 & 0.3383 & 0.0456 \\
...
GCN + BGE-M3 (rank fusion) & 1.52\% & 11.04\% & 31.47\% & 44.24\% & 0.0495 & 0.1187 & 0.4424 & 0.0478 \\
...
\begin{enumerate}
    \item \textbf{Node2Vec (uncorrected) dominates all other models} with Hit@10 = 16.06\% and Hit@100 = 64.33\%.  This remains the strongest result in the study and shows that applying degree correction to pure structural similarity \emph{hurts} link-prediction performance.

    \item \textbf{BGE-M3 semantic embeddings remain the strongest single non-structural signal.} BGE-M3 Only achieves 10.20\% Hit@10 and 0.0506 MRR, narrowly outperforming the corrected combined embedding and substantially outperforming corrected Node2Vec and both standalone GNN models by Hit@10.

    \item \textbf{Naive concatenation is highly sensitive to whether the structural component is degree-corrected.} The uncorrected combined embedding still performs catastrophically (0.24\% Hit@10), but the corrected combined embedding recovers to 9.82\% Hit@10 and 0.0471 MRR.  This suggests that direct concatenation is not universally broken, but without controlling the structural popularity signal it becomes unusable.
    ...
    \item \textbf{Degree correction backfires for pure Node2Vec, but helps stabilize the combined embedding.} Corrected Node2Vec (7.95\% Hit@10) performs much worse than uncorrected Node2Vec (16.06\% Hit@10), yet the corrected combined embedding (9.82\% Hit@10) is dramatically better than the uncorrected combined embedding (0.24\% Hit@10).  The penalty therefore appears harmful for pure structural retrieval but helpful when structural and semantic spaces are fused.

    \item \textbf{Exact rank fusion (GCN + BGE-M3) is competitive.} With 11.04\% Hit@10 and 44.24\% Hit@100, full-candidate Reciprocal Rank Fusion slightly improves Hit@10 over BGE-M3 alone (10.20\%) while trailing it slightly on MRR (0.0495 vs 0.0506).  This suggests that ranking-level fusion is a viable alternative to raw concatenation when heterogeneous spaces need to be combined.
\end{enumerate}
...
2 & GCN + BGE-M3 (rank fusion) & 11.04\% & 0.0495 \\
3 & BGE-M3 Only & 10.20\% & 0.0506 \\
4 & BGE-M3 + Node2Vec (corrected) & 9.82\% & 0.0471 \\
5 & Node2Vec (corrected) & 7.95\% & 0.0351 \\
6 & GCN Only & 6.20\% & 0.0302 \\
7 & GraphSAGE Only & 1.43\% & 0.0087 \\
8 & BGE-M3 + Node2Vec (uncorrected) & 0.24\% & 0.0019 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Rank fusion and semantic retrieval form the next-best tier.} Exact GCN+BGE-M3 rank fusion reaches 11.04\% Hit@10, while BGE-M3 alone reaches 10.20\%.  This shows that semantic information is highly useful and can be strengthened further by ranking-level structural fusion.

    \item \textbf{Degree correction hurts pure Node2Vec performance.} The power-law penalty reduces Node2Vec's Hit@10 from 16.06\% to 7.95\%. This suggests that hub nodes, despite their high connectivity, carry important structural information that should not be suppressed in the standalone structural model.

    \item \textbf{Uncorrected combined concatenation is broken, but corrected concatenation is competitive.} The uncorrected combined model collapses to 0.24\% Hit@10, whereas the corrected combined model reaches 9.82\% Hit@10.  This indicates that fusion by concatenation is only viable here when the structural component is explicitly damped before combination.
    ...
    \item \textbf{Exact rank fusion (GCN + BGE-M3) performs well.} With 11.04\% Hit@10 and 0.0495 MRR, full-candidate RRF clearly outperforms both uncorrected combined concatenation and corrected Node2Vec, and slightly exceeds the BGE-M3 baseline on Hit@10.  This suggests that ranking-level fusion is a more reliable way to combine heterogeneous spaces than raw vector concatenation.
\end{itemize}
...
embeddings (BGE-M3)}, while exact rank fusion between GCN and BGE-M3 forms a
strong second tier.  The surprising dominance of uncorrected Node2Vec
(Hit@10 = 16.06\%) over the semantic baseline (10.20\%) suggests that graph
topology alone captures more link-prediction signal than article text semantics.

The catastrophic failure of the \emph{uncorrected} combined embedding
(0.24\% Hit@10) highlights a fundamental challenge: directly joining
heterogeneous embedding spaces (1024-dim text + 128-dim structure) is highly
unstable when the structural component is left unnormalized.  However, the
degree-corrected combined embedding reaches 9.82\% Hit@10, showing that simple
concatenation is not inherently hopeless if the structural signal is first
controlled.  Future work should explore learned weighting or attention-based
fusion instead of fixed concatenation.
...
The exact rank fusion approach (GCN + BGE-M3) achieves 11.04\% Hit@10 and
44.24\% Hit@100, making it competitive with the best semantic baseline and far
better than uncorrected combined concatenation.  This suggests that ranking-level
fusion of heterogeneous spaces is a viable direction, and likely more robust than
raw vector concatenation.
