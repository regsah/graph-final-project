...
GCN + BGE-M3 (rank fusion) & 0.37\% & 3.49\% & 13.62\% & 23.22\% & 0.0181 & 0.0532 & 0.2322 & 0.0162 \\
...
\begin{enumerate}
    ...
    \item \textbf{Rank fusion (GCN + BGE-M3) achieves 3.49\% Hit@10} using top-500 optimization.  While substantially better than combined embedding concatenation (0.24\%), it still underperforms both individual models, suggesting that simple RRF merging of heterogeneous spaces is not optimal.
\end{enumerate}
...
5 & GCN + BGE-M3 (rank fusion) & 3.49\% & 0.0181 \\
6 & GraphSAGE Only & 1.43\% & 0.0087 \\
7 & BGE-M3 + Node2Vec (uncorrected) & 0.24\% & 0.0019 \\
8 & BGE-M3 + Node2Vec (corrected) & 0.05\% & 0.0004 \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{Rank fusion (GCN + BGE-M3) with top-500 optimization achieves 3.49\% Hit@10.} While substantially better than combined embedding concatenation (0.24\%), it underperforms both individual models. This suggests that simple RRF merging of rankings from heterogeneous embedding spaces is not optimal.
\end{itemize}
...
The rank fusion approach (GCN + BGE-M3 with top-500 optimization) achieves
3.49\% Hit@10, substantially better than combined embedding concatenation but
still underperforming both individual models.  This suggests that simple rank
fusion of heterogeneous spaces is not optimal; learned weighting or attention-based
fusion may be more effective.
