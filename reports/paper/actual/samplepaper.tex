...
Wikipedia is more than a collection of isolated articles; it is a large,
widely used, openly accessible knowledge environment in which rich page
content coexists with explicit hyperlink relations and broad topical coverage.
These properties have made Wikipedia a recurring resource for benchmark
construction, retrieval, graph-learning, and graph-text modeling
research~\cite{mernyei2020wikics,wikirecnet2020,wikigraphs2021}.
From a modeling perspective, this is appealing because the same article space
exposes both semantic content and relational context.

Within this environment, article-to-article recommendation is a natural and
useful problem. Given a page currently being examined, the goal is to recommend
other pages worth reading next. The hyperlink graph provides a strong starting
point because direct links often encode editorial, conceptual, or historical
relatedness~\cite{wikirecnet2020,grover2016node2vec}. However, graph structure
alone may not be sufficient. For instance, a recommendation process driven only
by hyperlink topology around an operating-systems article may emphasize
directly linked hub pages such as Linus Torvalds, while semantically close but
less directly connected pages on kernel design, Unix-like systems, or
file-system concepts can remain under-ranked. This suggests that page content
is not an auxiliary signal but a second source of evidence for recommendation.

This observation creates a practical gap in open Wikipedia benchmarks. On the
one hand, Wikipedia-derived graph resources such as Wiki-CS provide a
convenient and widely reused structural benchmark~\cite{mernyei2020wikics}. On
the other hand, Wiki-CS was introduced for node classification rather than
article recommendation, and it does not directly provide a text-complete,
up-to-date benchmark for regenerating article representations with modern text
encoders. Graph-text paired resources exist for other objectives as
well~\cite{wikigraphs2021}, but they do not by themselves resolve the need for
a recommendation-oriented, text-recoverable version of Wiki-CS. As a result,
there remains a gap between the availability of open Wikipedia graphs and the
availability of an open benchmark that supports joint structural and semantic
recommendation experiments on the same article set.
...
controlled topical setting in which recommendation consistency can be examined
...
graph topology, article content, or both. In particular, we investigate a
...
representations~\cite{grover2016node2vec}, alongside GCN and GraphSAGE
baselines~\cite{kipf2017semi,hamilton2017inductive}. We also examine corrected
structural variants and pooled semantic representations in order to better
balance structural and textual evidence. To assess whether the proposed hybrid
strategy is useful beyond a narrow baseline comparison, we evaluate it against
a broad spectrum of semantic, structural, hybrid, and GNN-based alternatives.
