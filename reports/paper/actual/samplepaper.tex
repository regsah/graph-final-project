...
This is also what makes the Wikipedia setting scientifically interesting.
Openly available graph extractions make structural experimentation feasible, and
resources such as WikiLinkGraphs and Wiki-CS provide convenient benchmark
substrates for studying Wikipedia connectivity at scale~\cite{consonni2019wikilinkgraphs,mernyei2020wikics}. However, the open
...
yet they do not by themselves provide a recommendation-oriented, text-complete
version of the same Wiki-CS article set. As a result, there remains a gap
...
offers the most balanced top-list recommendation behavior in the present
setting. The pooled corrected hybrid reaches the top Hit@10 value, while the
non-pooled corrected hybrid remains strong across MRR, NDCG, Recall, and
several broader quality measures. These models benefit from access to both
text-level semantic proximity and graph-level structural context, without
...
Several limitations should also be kept in view. First, the evaluation protocol
is based on held-out edge recovery. The absence of a hyperlink does not imply
that an article is irrelevant, and real reader behavior is not observed
directly.
Second, the undirected view improves comparability across structural and hybrid
families, but it simplifies editorial links that are inherently directed.
Third, both degree correction and pooling are heuristic engineering choices
rather than learned fusion mechanisms. Finally, although the benchmark is
text-complete after reconstruction, recovered raw text may still contain
link-related cues or title mentions that should be kept in mind when
interpreting edge-recovery performance.
...
revised evaluation indicates that corrected semantic-structural hybrids are the
most balanced family for recommendation-oriented top-list retrieval under the
revised evaluation protocol, while semantic-only and structural-only models remain
competitive on specific metrics such as Hit@1, MAP@10, ROC-AUC, and PR-AUC.
This suggests that hybrid retrieval is a useful direction when the target task
emphasizes short ranked recommendation lists rather than graph prediction in
isolation.

Future work can extend this line in several directions, including broader
Wikipedia domains, directed evaluation protocols, harder negative sampling,
lexical and graph-heuristic baselines, alternative structural correction
schemes, and richer hybrid fusion strategies that preserve interpretability
...
the scope of project number FBA-2026-1345 , and titled ``Çok Modlu Öneri Sistemlerinde Doğruluk, Çeşitlilik, Adalet ve''.
