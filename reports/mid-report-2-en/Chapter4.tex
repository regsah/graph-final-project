...
\textbf{Layer 3 -- Recommendation and Presentation:} Given a user query, the system encodes the query with the same BGE-M3 model and computes cosine similarity against all pre-computed node embeddings. The top-$k$ most similar articles are retrieved and passed to a local LLM (DeepSeek-R1 via Ollama) to generate a hierarchical learning path, which is then served to the user.
...
\begin{enumerate}
    ...
    \item \textbf{Similarity Search:} Cosine similarity is computed between the query vector and all pre-cached degree-normalised node embeddings. The top-$k$ nearest nodes in the embedding space are identified.
    ...
    \item \textbf{LLM Structuring} \textit{[planned]:} The ranked flat list is passed to a local LLM (DeepSeek-R1 via Ollama), which organises the articles into a hierarchical prerequisite structure.
...
The Graph Attention Network (GAT) layer extends the standard graph convolution by computing attention coefficients that weigh the importance of each neighbor. Given node features $\mathbf{h} = \{\vec{h}_1, \vec{h}_2, \ldots, \vec{h}_N\}$ where $\vec{h}_i \in \mathbb{R}^F$, the attention mechanism proceeds as follows:
...
\subsection{Training Flowchart}

Figure~\ref{fig:training-flow} presents the training procedure flowchart for the GNN-based link prediction model.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\textwidth]{figures/training-flowchart.pdf}
    \caption{Training flowchart: GNN link prediction with positive/negative sampling.}
...
\begin{enumerate}
    \item Load the WikiCS dataset and split edges into train/validation/test sets
    \item Initialize the GNN model (GAT or GCN) and optimizer (Adam, lr=0.01)
    \item For each epoch:
    \begin{enumerate}
        \item Generate node embeddings via forward pass: $\mathbf{Z} = \text{GNN}(\mathbf{X}, \mathbf{A})$
        \item Sample negative edges (random unconnected node pairs)
        \item Compute link prediction scores via dot product decoder: $\hat{y}_{ij} = \sigma(\vec{z}_i^T \vec{z}_j)$
        \item Compute Binary Cross-Entropy loss: $\mathcal{L} = \text{BCE}(\hat{y}, y)$
        \item Backpropagate and update model parameters
    \end{enumerate}
    \item Evaluate on validation set every 10 epochs
    \item Apply early stopping if validation AUC does not improve
    \item Report final test AUC on held-out test edges
\end{enumerate}

\subsection{Inference Flowchart}

Figure~\ref{fig:inference-flow} presents the inference procedure for generating article recommendations.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\textwidth]{figures/inference-flowchart.pdf}
    \caption{Inference flowchart: query to top-k recommendations.}
...
\begin{enumerate}
    \item Receive user query string
    \item Encode query using the same sentence embedding model: $\vec{q} = \text{Encode}(query)$
    \item Compute cosine similarity between $\vec{q}$ and all node embeddings $\{\vec{z}_i\}$
    \item Rank nodes by similarity score in descending order
    \item Retrieve top-k article titles and metadata
    \item (Optional) Pass top-k articles to LLM for hierarchical organization
    \item Return structured recommendations to user
\end{enumerate}
