...
\midrule
GPU (RTX 4070 Laptop) & 140W (peak) / 100W (avg) & 15--20 min \\
System (CPU, RAM, SSD) & 60W & 15--20 min \\
Total & 160--200W (avg ~160W) & 15--20 min \\
\midrule
...
Community detection using Louvain and Infomap algorithms reveals meaningful topical clusters in the graph (Figures~\ref{fig:louvain-communities} and \ref{fig:infomap-communities}, and Table~\ref{tab:community-results}).
...
Both algorithms confirm that the graph possesses meaningful community structure. Louvain produces a small number of large communities (mean $\approx$299 nodes), while Infomap finds many smaller, finer-grained communities (mean $\approx$43 nodes). Despite the difference in granularity, both algorithms agree on the key finding (outlined in Table~\ref{tab:sim-community}): nodes within the same community share consistently higher cosine similarity than nodes in different communities, validating the structural partitioning.

\subsection{GCN Baseline Link Prediction Results}

The first GNN experiments were conducted using a Graph Convolutional Network (GCN) baseline for the link prediction task, as summarized in Table~\ref{tab:gcn-results}.
...
A content-based recommendation system combining BGE-M3 embeddings and SVD-based Node2Vec was evaluated on 10 broad CS topic queries (Report-5), with sample results presented in Table~\ref{tab:recommendation-results}.
