...
\midrule
WikiCS raw data (PyG format) & 21.6 MB \\
BGE-M3 embeddings (11,367 $\times$ 1024 $\times$ float32) & 46.6 MB \\
Node2Vec embeddings (11,367 $\times$ 128 $\times$ float32) & 5.8 MB \\
Combined embedding cache & 52.4 MB \\
Adjacency matrix (sparse) & 8.2 MB \\
Total cached data & $\approx$ 80 MB \\
\bottomrule
\end{tabular}
\end{table}

The embedding storage dominates the dataset size, accounting for approximately 65\% of total cached data. Pre-computing embeddings offline saves significant recomputation time.
...
\midrule
BAAI/bge-m3 (1024-dim) & 30--60 sec & RTX 4070 GPU \\
mGTE-modernbert (768-dim) & 20--40 sec & RTX 4070 GPU \\
...
Prior to GNN training, a comprehensive structural analysis of the Custom-WikiCS graph was performed (Report-4). Key findings are summarized below.
...
\begin{figure}[H]
    ...
    \caption{Louvain community size distribution (mean community size: 31.4 nodes).}
...
\begin{figure}[H]
    ...
    \caption{Infomap community size distribution (mean community size: 19.6 nodes).}
...
\midrule
Modularity             & 0.6504 & 0.5998 \\
Number of communities  & 373    & 596    \\
Max community size     & 1,971  & 837    \\
Mean community size    & 31.37  & 19.63  \\
\bottomrule
\end{tabular}
\end{table}
...
Both algorithms confirm that the graph possesses strong community structure with semantically coherent clusters. Nodes within the same community share consistently higher cosine similarity than nodes in different communities, validating the structural partitioning.
...
\begin{table}[htbp]
\centering
\caption{GCN training dynamics over epochs.}
\label{tab:gcn-results}
\begin{tabular}{rrrr}
\toprule
\textbf{Epoch} & \textbf{Loss} & \textbf{Val AUC} & \textbf{Notes} \\
\midrule
10 & 0.619 & 0.887 & Best validation AUC \\
20 & 0.596 & 0.871 & \\
50 & 0.580 & 0.859 & Overfitting begins \\
100 & 0.559 & 0.843 & Overfitted \\
\midrule
\textbf{Test AUC (final)} & \multicolumn{3}{r}{\textbf{0.8407}} \\
\bottomrule
\end{tabular}
\end{table}

The GCN model rapidly achieves strong validation AUC (~0.887) by epoch 10, but subsequently overfits: the validation AUC decreases from 0.887 to 0.843 as training continues to epoch 100. This suggests that early stopping at epoch 10--20 would yield optimal results.

The final test AUC of \textbf{0.8407} establishes a reasonable baseline for the link prediction task.
...
\begin{itemize}
    ...
    \item The GCN baseline achieves Test AUC = 0.8407, establishing a solid baseline for link prediction
    \item Early stopping is essential: the GCN overfits after epoch 10, suggesting regularization or architectural improvements are needed
