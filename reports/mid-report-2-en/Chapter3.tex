...
\begin{itemize}
    \item \textbf{Modern sentence embedding models} (BAAI/bge-m3) to capture the semantic content of article texts as high-dimensional dense vectors
    \item \textbf{Graph-structural embeddings} (SVD-based Node2Vec via PPMI matrix factorisation) to encode each article's neighbourhood context and position in the hyperlink graph
    \item \textbf{Large Language Models (LLMs)} to organize recommended articles into hierarchical learning paths
\end{itemize}

The system treats Wikipedia's hyperlink structure as a directed graph where nodes represent articles and directed edges represent hyperlinks. Text and structural embeddings are concatenated per node and a degree-normalised popularity penalty is applied to prevent hub articles from dominating recommendations. Graph Attention Networks (GAT)~\cite{velickovic2018graph} are under active investigation as a planned extension to further enrich node representations.
...
\begin{itemize}
    ...
    \item Using an LLM inference (Ollama) instead of paid API services (OpenAI, Anthropic)
...
\begin{itemize}
    \item \textbf{Training efficiency:} All GNN models are designed to converge within 100 epochs, with early stopping to prevent unnecessary computation. A typical training run consumes approximately 0.05--0.10 kWh.
...
\begin{itemize}
    ...
    \item \textbf{Transparency:} The recommendation logic is based on cosine similarity of pre-computed embeddings, providing a clear and auditable recommendation signal. Future GAT-based extensions will offer additional interpretability through attention weights.
...
Ethics \& Interpretability & Query-based only, no user profiling, similarity-based auditable recommendations \\
